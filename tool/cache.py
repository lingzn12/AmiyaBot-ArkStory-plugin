...
class CacheTow:
    cached_data: Dict[str, Dict[str, Union[dict, float]]] = dict()  # 更新下载

    @classmethod
    @lru_cache(maxsize=None)
    def get_cache(cls, local_path: Path) -> Optional[dict]:
        # 获取数据名称
        data_name = os.path.splitext(os.path.basename(local_path))[0]
        # 如果数据名称已经在缓存中，则判断数据是否过期
        if data_name in cls.cached_data:
    ...
    @classmethod
    def set_cache(cls, local_path: Path, data_name: str) -> Optional[dict]:
        # 读取数据
